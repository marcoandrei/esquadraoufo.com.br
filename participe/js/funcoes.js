function mostraAba(evt, nomeAba) {
  ...
  // Get all elements with class="tablinks" and remove the class "ativo"
  tablinks = document.getElementsByClassName("aba");
  for (i = 0; i < tablinks.length; i++) {
    tablinks[i].className = tablinks[i].className.replace(" ativo", "");
  }

  // Show the current tab, and add an "ativo" class to the button that opened the tab
  document.getElementById(nomeAba).style.display = "block";
  evt.currentTarget.className += " ativo";
}

function mostraAbaId(idElemento, nomeAba) {
  // Declare all variables
  var i, tabcontent, tablinks;
...
// Função que confirma o envio do projeto

function confirmaEnvioProjeto(id) {
  if (
    confirm(
      "ATENÇÃO! VOCÊ ESTÁ ENVIANDO SEU PROJETO PARA ANÁLISE! \n\nTem certeza que preencheu todos os dados e anexou os documentos solicitados? \n\nSe continuar, não será mais possível fazer qualquer ajuste.\n\nPROJETOS COM INFORMAÇÕES INCOMPLETAS SERÃO AUTOMATICAMENTE REPROVADOS.\n\nCaso queira revisar seu projeto, clique em Cancelar."
    )
  ) {
    document.location = "entidade-envia-projeto.php?id=" + id;
  }
}

// Função que valida a gravação de projetos
function validaDadosEnviaProjeto() {
  //Variáveis do form
  var nome_projeto = document.form_projeto_cadastro.projeto_nome.value;
  var projeto_resp = document.form_projeto_cadastro.projeto_resp.value;
  var projeto_resp = document.form_projeto_cadastro.projeto_resp.value;

  // Variáveis de erro
  var nomeProjErr = true;
  var respProjErr = true;
  ...
  if (nome_projeto == null || nome_projeto == "") {
    mostraErro(
      "nomeProjErr",
      "O responsável pelo é uma informação obrigatória."
    );
    document.getElementById("projeto-nome").focus();
  } else {
    mostraErro("nomeProjErr", "");
  ...
  // Impede do formulário ser enviado se houver erros
  if ((nomeProjErr || respProjErr) == true) {
    return false;
  }
}
